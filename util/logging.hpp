...
#define CHECK(x)                                         \
    if (!(x))                                            \
    my_log::LogMessageFatal(__FILE__, __LINE__).stream() \
            << "Check failed: " #x << ' '
...
#define CHECK_NOTNULL(x) \
    ((x) == NULL ? my_log::LogMessageFatal(__FILE__, __LINE__).stream() << "Check  notnull: " #x << ' ', (x) : (x))// NOLINT(*)
// Debug-only checking.
#ifdef NDEBUG
#define DCHECK(x) \
    while (false) CHECK(x)
...
#define LOG_INFO my_log::LogMessage(__FILE__, __LINE__)

#define LOG_ERROR LOG_INFO
#define LOG_WARNING LOG_INFO
#define LOG_FATAL my_log::LogMessageFatal(__FILE__, __LINE__)
...
#define LOG_IF(severity, condition) \
    !(condition) ? (void) 0 : my_log::LogMessageVoidify() & LOG(severity)

#ifdef NDEBUG
#define LOG_DFATAL LOG_ERROR
#define DFATAL ERROR
#define DLOG(severity) true ? (void) 0 : my_log::LogMessageVoidify() & LOG(severity)
#define DLOG_IF(severity, condition) \
    (true || !(condition)) ? (void) 0 : my_log::LogMessageVoidify() & LOG(severity)
#else
